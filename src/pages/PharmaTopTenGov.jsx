...
const PharmaTopTenGov = () => {
  ...
  useEffect(() => {
    const fetchColleges = async () => {
      try {
        ...
        // ✅ Filter only pharma + private
        const pharmaPrivate = collegesData.filter(
          college =>
            college.category?.toLowerCase() === 'pharma' &&
            college.ownership?.toLowerCase() === 'government'
        );

        // ✅ Take only top 10
        const top10 = pharmaPrivate.slice(0, 10);

        setColleges(top10);
        setFilteredColleges(top10);

        // ✅ Extract filter options from top 10
        const states = [...new Set(top10.map(c => c.state || ''))].filter(Boolean);
        const cities = [...new Set(top10.map(c => c.city || ''))].filter(Boolean);
        const ownerships = [...new Set(top10.map(c => c.ownership || ''))].filter(Boolean);
        const courses = [...new Set(top10.flatMap(c =>
  ...
  useEffect(() => {
    const filtered = colleges.filter(college => {
      const hasCourse = filters.course === '' ||
        (Array.isArray(college.coursesAndFees) &&
          college.coursesAndFees.some(cf => cf.name === filters.course));
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-6 sm:mb-8">
          Top 10 Giovernment Pharmacy Colleges in India
        </h1>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm mb-8 border border-gray-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <FilterSelect label="State" name="state" value={filters.state} options={filterOptions.states} onChange={handleFilterChange} />
            <FilterSelect label="City" name="city" value={filters.city} options={filterOptions.cities} onChange={handleFilterChange} />
            <FilterSelect label="Ownership" name="ownership" value={filters.ownership} options={filterOptions.ownerships} onChange={handleFilterChange} />
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Category</label>
              <input type="text" value="Pharma" readOnly
                className="w-full p-2.5 border border-gray-300 rounded-lg bg-gray-50 cursor-not-allowed" />
            </div>
            <FilterSelect label="Course" name="course" value={filters.course} options={filterOptions.courses} onChange={handleFilterChange} />
          </div>
        </div>

        {/* Results Count */}
        <div className="mb-6 text-gray-600 text-sm sm:text-base">
          Showing {filteredColleges.length} of {colleges.length} private pharmacy colleges
        </div>
    ...
    >
      <div className="p-6 sm:p-7">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <Link to='/connect'>
            <button className="text-blue-600 hover:text-blue-800 font-medium text-sm sm:text-base px-3 py-1.5 cursor-pointer rounded-md hover:bg-blue-50 transition-colors">
              Download Brochure
            </button>
          </Link>
        </div>
...
export default PharmaTopTenGov;
