  ...
  useEffect(() => {
    const fetchColleges = async () => {
      try {
        ...
        // ✅ Filter only pharma category
        const pharmaColleges = collegesData.filter(
          college => college.category?.toLowerCase() === 'pharma'
        );

        setColleges(pharmaColleges);
        setFilteredColleges(pharmaColleges);

        // ✅ Extract filter options
        const states = [...new Set(pharmaColleges.map(c => c.state || ''))].filter(Boolean);
        const cities = [...new Set(pharmaColleges.map(c => c.city || ''))].filter(Boolean);
        const ownerships = [...new Set(pharmaColleges.map(c => c.ownership || ''))].filter(Boolean);
        const courses = [...new Set(pharmaColleges.flatMap(c =>
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 text-gray-600 text-sm sm:text-base">
          Showing {filteredColleges.length} of {colleges.length} pharmacy colleges
        </div>
