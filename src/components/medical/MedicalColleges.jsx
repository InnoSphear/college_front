 ...
 useEffect(() => {
  const fetchColleges = async () => {
    try {
      ...
      // Filter only medical category
      const medicalColleges = collegesData.filter(college => 
      ...
      // ✅ Get unique filter values from new schema
      const states = [...new Set(medicalColleges.map(c => c.state || ''))].filter(Boolean);
      const cities = [...new Set(medicalColleges.map(c => c.city || ''))].filter(Boolean);
      const ownerships = [...new Set(medicalColleges.map(c => c.ownership || ''))].filter(Boolean);
      
      // ✅ Extract course names from coursesAndFees array
      const courses = [...new Set(medicalColleges.flatMap(c => 
